...
const validateRegulatoryRequestCode = `// Canonical source for PII-07 "Validate Regulatory Request" Code node.
...
return results;
`;
const evaluateRegulatoryCatalystCode = `// Canonical source for PII-07 "Evaluate Regulatory Catalyst" Code node.
...
`;
const expandRegulatoryClaimsCode = `// Expand Evaluate Regulatory Catalyst claims[] into one item per claim upsert.
...
`;
const expandRegulatoryClaimLinksCode = `// After regulatory claim inserts, expand claim_evidence_links rows.
...
return links;
`;
const prepareRegulatoryAggregateCode = `// Prepare aggregate after regulatory claim/link upserts.
...
const evaluated = nodeJson('Evaluate Regulatory Catalyst') || $input.first().json || {};
let claimStored = 0;
try {
  claimStored = $('Insert Regulatory Claims')
    .all()
...
`;
const buildRegulatoryResultCode = `// Canonical source for PII-07 "Build Regulatory Result" Code node.
