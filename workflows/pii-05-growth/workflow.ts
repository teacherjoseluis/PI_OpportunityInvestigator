...
const validateGrowthRequestCode = `// Canonical source for PII-05 "Validate Growth Request" Code node.
...
return results;
`;
const evaluateGrowthProspectsCode = `// Canonical source for PII-05 "Evaluate Growth Prospects" Code node.
...
`;
const expandGrowthClaimsCode = `// Expand Evaluate Growth Prospects claims[] into one item per claim upsert.
...
return claims.map((claim, index) => ({
  json: {
    ...
    claim_category: claim.claim_category || 'growth_prospects',
...
`;
const expandGrowthClaimLinksCode = `// After growth claim inserts, expand claim_evidence_links rows.
...
const expanded = nodeAll('Expand Growth Claims').filter(
...
if (!links.length) {
  const evalRow = (() => {
    try {
      return $('Evaluate Growth Prospects').first().json;
    } catch {
...
return links;
`;
const prepareGrowthAggregateCode = `// Prepare aggregate after growth claim/link upserts.
...
const evaluated = nodeJson('Evaluate Growth Prospects') || $input.first().json || {};
let claimStored = 0;
try {
  claimStored = $('Insert Growth Claims')
    .all()
...
`;
const buildGrowthResultCode = `// Canonical source for PII-05 "Build Growth Result" Code node.
