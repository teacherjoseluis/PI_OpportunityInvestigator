...
describe('evaluate-valuation-market', () => {
  ...
  it('emits net_cash_debt fact when XBRL financial metrics are present', () => {
    const [result] = runCodeNode(EVALUATE, {
      items: [{ valuation_config: valuationConfig }],
      nodes: {
        'Validate Valuation Request': [
    ...
    assert.equal(result.json.outcome, 'ANALYZED');
    assert.ok(!result.json.insufficient_topics.includes('net_cash_debt'));
    const netClaim = result.json.claims.find((c) => c.topic_key === 'net_cash_debt');
    assert.ok(netClaim);
    assert.equal(netClaim.extraction_method, 'deterministic_xbrl_metrics');
